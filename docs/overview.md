# stirlingpdf-agent — Concept Overview

> **Category**: Productivity | **Ecosystem Role**: MCP Server + A2A Agent
...
```json
{
  "mcpServers": {
    "stirlingpdf-agent": {
      "command": "uv",
      "args": ["run", "--with", "stirlingpdf-agent", "stirlingpdf-mcp"],
      "env": {}
    }
  }
}
...
```bash
stirlingpdf-mcp --transport streamable-http --port 8001
```
